...
| Tests | 683 Python tests at 95% line coverage (CI runs the suite on Python 3.12, 3.13 and 3.14); 79 web unit tests (also run under four timezones); 48 real-browser tests (24 per mode); 45 Android unit tests plus lint; ruff, ruff-format and mypy clean; `pip-audit` and `npm audit` report no known vulnerabilities; no secrets in the tree or git history | VERIFIED |
