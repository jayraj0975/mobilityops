...
| Web console: 81 unit and component tests, 24 real-browser tests including WCAG A/AA scans of every page in both themes, offline recovery, phone width | VERIFIED |
| Android app (Kotlin screens; debug APK, signed release APK, AAB): 45 unit tests, lint clean, run on Android 14 and Android 16 emulators against the live server, including the server being killed and restarted | VERIFIED on an emulator; **not** on a physical phone |
...
| Tests | 683 Python tests at 95% line coverage (CI runs the suite on Python 3.12, 3.13 and 3.14); 81 web unit tests (also run under four timezones); 48 real-browser tests (24 per mode); 45 Android unit tests plus lint; ruff, ruff-format and mypy clean; `pip-audit` and `npm audit` report no known vulnerabilities; no secrets in the tree or git history | VERIFIED |
