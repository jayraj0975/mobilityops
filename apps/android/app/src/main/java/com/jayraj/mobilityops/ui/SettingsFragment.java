...
import com.jayraj.mobilityops.MainActivity;
    ...
    protected void build(@NonNull Context c) {
        ...
        save.setOnClickListener(v -> {
            ...
            load(() -> {
                ...
                String mode = meta.optString("mode");
                return new String[] {mode, meta.optString("data_label") + " (" + mode + " mode), "
                        + meta.optInt("n_zones") + " zones, "
                        + (services == null || services.length() == 0 ? 1 : services.length()) + " service(s), API "
                        + meta.optString("api_version")};
            }, ok -> {
                result.setText("Connected: " + ok[1] + ".");
                // Remember the mode and switch the tabs now, not at the next launch.
                settings.saveMode(ok[0]);
                if (getActivity() instanceof MainActivity) {
                    ((MainActivity) getActivity()).applyMode("pune".equals(ok[0]));
                }
            },
        ...
        if ("pune".equals(settings.mode())) {
            content.addView(Ui.body(c, "MobilityOps " + BuildConfig.VERSION_NAME + ". A real-time view of Pune: weather, "
                    + "air quality and rain from public sources, with SIMULATED mobility demand, forecasts with ranges "
                    + "and events."));
            content.addView(Ui.muted(c, "No open source of Pune trip data exists, so demand is generated by a model and "
                    + "labelled SIMULATED. Weather data by Open-Meteo.com and MET Norway (CC BY 4.0); map data \u00a9 "
                    + "OpenStreetMap contributors (ODbL). Forecasts are estimates, not guarantees."));
        } else {
